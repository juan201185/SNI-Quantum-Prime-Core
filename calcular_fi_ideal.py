...
def generar_features(dataset_f_ideal):
    """
    Genera un vector de características (features) BASADO EN INERCIA.
    Utiliza la posición actual (X_actual) y el primo anterior (P_X_anterior) 
    para predecir el F_ideal_actual, eliminando la fuga de datos.
    INCLUYE ESCALADO DE CARACTERÍSTICAS para estabilidad de la RN.
    """
    global scaler_features 
    
    print("\n--- Iniciando Ingeniería de Características (CON DESFASE INERCIAL) ---")
    ...
    # Empezamos el bucle desde el índice 1 para poder mirar hacia atrás (i-1)
    for i in range(1, len(dataset_f_ideal)):
        entrada_actual = dataset_f_ideal[i]
        entrada_anterior = dataset_f_ideal[i-1]
        
        X_actual = entrada_actual["X"]
        F_ideal_actual = entrada_actual["F_ideal"] # El objetivo real a predecir
        
        # --- LA INERCIA: Capturamos el primo inmediatamente anterior ---
        P_X_anterior = entrada_anterior["P_X"] 
        
        # --- CÁLCULO DE CARACTERÍSTICAS (Sellando la fuga de datos) ---
        feature_X = float(X_actual)
        feature_ln_X = math.log(X_actual) 
        feature_X_squared = float(X_actual**2)
        feature_sqrt_X = float(math.sqrt(X_actual)) 
        feature_one_over_X = 1.0/X_actual 
        feature_nd_X = float(int(math.log10(X_actual)) + 1) 
        feature_X_cubed = float(X_actual**3) 

        # Usamos estrictamente el P_X_anterior en lugar del P_X actual
        feature_P_X_ant = float(P_X_anterior) 
        feature_ln_P_X_ant = math.log(P_X_anterior) 
        
        feature_log10_X = math.log10(X_actual) if X_actual > 0 else 0.0 
        feature_log10_PX_ant = math.log10(P_X_anterior) if P_X_anterior > 0 else 0.0 
        feature_pow10_nd_X = 10**(int(math.log10(X_actual))) if X_actual > 0 else 1.0 
        feature_pow10_nd_PX_ant = 10**(int(math.log10(P_X_anterior))) if P_X_anterior > 0 else 1.0
        ...
        features_vector = [
            ...
            feature_nd_X,
            feature_X_cubed,
            feature_P_X_ant,         # Histórico inercial
            feature_ln_P_X_ant,      # Histórico inercial
            feature_log10_X,      
            feature_log10_PX_ant,    # Histórico inercial
            feature_pow10_nd_X,   
            feature_pow10_nd_PX_ant  # Histórico inercial
        ]
        
        raw_features_list.append(features_vector)
        y_targets_list.append(F_ideal_actual) 

        primos_procesados += 1
        if primos_procesados % 1000000 == 0:
            print(f"  {primos_procesados} puntos procesados para características...")
    ...
    print(f"\n--- Generación inercial y ESCALADO completados para {len(dataset_con_features)} puntos ---")
